...
def solicitar_exames(request):
    ...
    if request.method == 'GET':
        return render(request,'solicitar_exames.html',{'tipos_exames':tipos_exames,'dia':dia,'mes':mes})
        ...
        for i in solicitacao_exames:
            if i.disponivel:
                preco_total += i.preco
        return render(request,'solicitar_exames.html',{'tipos_exames':tipos_exames,'solicitacao_exames':solicitacao_exames,'preco_total':preco_total,'dia':dia,'mes':mes})
...
def cancelar_pedido(request,pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request,constants.ERROR,"Esse pedido não é seu, então você não pode cancelar ele.")
        return redirect('/exames/gerenciar_pedidos/')
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        if not exame.usuario == request.user:
            messages.add_message(request,constants.ERROR,"Esse exame não é seu.")
            return redirect('/exames/gerenciar_exames')
        if not exame.resultado:
            messages.add_message(request,constants.ERROR,"Não existe um anexo nesse exame.")
            return redirect('/exames/gerenciar_exames')
...
def acesso_medico(request,token):
    ...
    if acesso_medico.status == "Expirado":
        messages.add_message(request,constants.ERROR,"Esse Token Já expirou, Solicite outro ao paciente")
